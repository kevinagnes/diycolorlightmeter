...
float getLux()
{
  ...
  if ((event.light == 0) |
    ...
    (event.light <-4294966000.0))
    {lux = 201;Overflow = 1;} else {Overflow = 0;}
    
  lux = lux * DomeMultiplier; // DomeMultiplier = 2.17
  if (lux < 40) {needHigh = 1;} // 1 +1/3 de erro
  if (lux > 200) {needHigh = 0;}
  if (needHigh == 1) {lux = lux*.26;}
  return lux;
...

{
    ...
    //////////////////////////// PHOTO MODE //////////////////////
  {
    ...
    if (Overflow == 1) {display.setCursor(10, 29);display.println(F("Overflow"));}
    t=0;
    refreshtiming();
  }
}
...
void introanimation() {
  ...
  display.println(F(" EXPOSURE METER v3.3 "));
